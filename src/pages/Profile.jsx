...
import { User, ShoppingBag, Clock, CheckCircle, Truck, Package, ShieldCheck } from "lucide-react";
...
export default function Profile() {
  ...
  const getFulfillmentBadge = (order) => {
    ...
    const fulfillmentState = order.fulfillmentStatus || (order.status !== "Paid" ? order.status : "Pending");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 font-body space-y-10 animate-fadeIn">
      ...
      <div className="bg-white border border-orange-100 p-6 rounded-3xl shadow-sm flex flex-col sm:flex-row items-center gap-4">
        <div className="w-16 h-16 bg-mango/10 rounded-2xl flex items-center justify-center text-mango"><User size={32} /></div>
        <div className="text-center sm:text-left space-y-0.5">
          <h2 className="text-2xl font-heading font-bold text-brown">{user.displayName || "Bliss Customer"}</h2>
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-heading font-bold text-brown flex items-center gap-2"><ShoppingBag size={20} /> Your Orchard Orders</h3>
        
        {loading ? (
          <div className="bg-white rounded-3xl p-12 text-center text-gray-400 font-medium animate-pulse border border-orange-50">Syncing Transaction Ledgers...</div>
        ) : orders.length === 0 ? (
          <div className="bg-white rounded-3xl border border-dashed border-orange-200 p-12 text-center text-gray-400 text-sm">You haven't initialized any bulk mango purchases yet!</div>
        ) : (
          <div className="space-y-4">
            {orders.map(order => (
              <div key={order.id} className="bg-white border border-orange-100 p-5 rounded-2xl shadow-xs space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-50 pb-3 gap-3">
                  <div>
                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Invoice Code:</span>
                    <p className="font-mono text-sm font-bold text-brown uppercase">{order.id.substring(0, 10)}...</p>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block text-left sm:text-right">Purchase Date:</span>
                    <p className="text-xs text-gray-500 font-semibold">{new Date(order.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
                ...
                {/* Items Mapping Sub-Row Block */}
                <div className="space-y-2">
                  {order.items?.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm font-medium">
                      <span className="text-gray-600">{item.title} <span className="text-xs font-bold text-mango-dark">x{item.qty}</span></span>
                      <span className="text-brown font-bold">₹{item.price * item.qty}</span>
                    </div>
                  ))}
                </div>

                <div className="pt-3 border-t border-gray-50 flex justify-between items-center text-sm font-bold">
                  <span className="text-gray-400 uppercase tracking-wider text-xs">Total Remittance:</span>
                  <span className="text-lg text-brown font-heading font-extrabold">₹{order.grandTotal}</span>
                </div>
              </div>
            ))}
          </div>
