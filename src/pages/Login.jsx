...
// The above code is for production level and below is for development testing only.

// D:\Portfoltio_Projects\mango-store\src\pages\Login.jsx
import React, { useState, useEffect } from "react";
import { Link, useNavigate, useLocation } from "react-router-dom";
import { auth, db } from "../firebase/config";
import { signInWithEmailAndPassword, signInWithPopup, GoogleAuthProvider } from "firebase/auth";
...
export default function Login() {
  ...
  const handleEmailLogin = async (e) => {
    if (e) e.preventDefault(); // Made conditional to handle custom trigger pipeline executions
    if (!email || !password) {
      toast.error("Please supply account credentials.");
      return;
    }
  ...
  // 🔐 RECRUITER BYPASS SYSTEM 
  const handleRecruiterQuickLogin = async () => {
    setSubmitting(true);
    toast.loading("Bypassing credentials gate...", { id: "recruiterAuth" });
    
    try {
      // 1. Programmatically inject the admin email and credential variables into local state fields
      const targetEmail = "w@gmail.com";
      const targetPassword = "qwerty"; // Ensure this matches your account password in Firebase Console
      
      setEmail(targetEmail);
      setPassword(targetPassword);

      // 2. Direct background request through the core execution gate
      await signInWithEmailAndPassword(auth, targetEmail, targetPassword);
      toast.success("Welcome, Recruiter! Admin Access Granted. 🎉", { id: "recruiterAuth" });
    } catch (err) {
      console.error(err);
      toast.error("Bypass rejected. Ensure the account password is correctly configured.", { id: "recruiterAuth" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 bg-[#FFFDF5] font-body animate-fadeIn space-y-4">
      
      {/* 👑 PORTFOLIO SHOWCASE: RECRUITER QUICK-LOGIN COMPONENT */}
      <div className="w-full max-w-md bg-amber-50/80 border border-amber-200/80 rounded-2xl p-4 text-center space-y-2 shadow-xs">
        <div className="flex items-center justify-center gap-1.5 text-amber-800 font-bold uppercase tracking-wider text-xs">
          <ShieldCheck size={14} /> Recruiter Evaluation Desk
        </div>
        <p className="text-[11px] text-amber-700 font-medium leading-relaxed">
          Skip standard sign-up forms. Click below to test live metrics tracking screens and coupon validations immediately.
        </p>
        <button 
          onClick={handleRecruiterQuickLogin}
          type="button"
          disabled={submitting}
          className="w-full bg-brown hover:bg-brown/90 text-white text-xs font-bold py-2.5 px-4 rounded-xl transition shadow-xs cursor-pointer disabled:opacity-50"
        >
